...
async function verify({ sandbox, repoDir, paths, log }) {
  ...
  const tests = await sh('sh', [
    '-c',
    'node ambient-explainer-extension/tests/providers.mjs && node ambient-explainer-extension/tests/voice-coach.mjs && node ambient-explainer-extension/tests/voice-frame.mjs && node ambient-explainer-extension/tests/voice-grammar.mjs',
  ]);
  if (tests.exitCode !== 0) {
