...
import { tellmeDb, tellmeNotes, tellmeAddNote } from '@/lib/tellme';
...
// Writing is not coding, and the same model is rarely best at both. Opus writes
// the patch; Sonnet writes the sentences a human reads — the explanation of why
// nothing was changed, and the short public account of how it ended. Separating
// them is also cheaper, since prose is the frequent call and code is the rare
// one.
const WRITING_MODEL = process.env.JC_WRITING_MODEL || 'anthropic/claude-sonnet-5';
...
async function verify({ sandbox, repoDir, paths, log }) {
  ...
  const tests = await sh('sh', [
    '-c',
    'node ambient-explainer-extension/tests/providers.mjs && node ambient-explainer-extension/tests/voice-coach.mjs && node ambient-explainer-extension/tests/voice-frame.mjs && node ambient-explainer-extension/tests/voice-grammar.mjs && node ambient-explainer-extension/tests/onboarding.mjs',
  ]);
  if (tests.exitCode !== 0) {
...
export async function runAgent({ runId, report }) {
  ...
  try {
    ...
    if (autonomy === 'diagnose') {
      const text = await withModelFallback(WRITING_MODEL, async (model) => {
        const result = await generateText({
          model: modelFor(model),
          system:
    ...
    // What was said since the last run. This is the whole point of the thread:
    // a previous run that stopped and asked "which text was in the pill?" gets
    // the answer here, so the re-run starts where the last one gave up rather
    // than repeating the same dead end.
    const thread = await tellmeNotes(report.id);
    if (thread.length) await log.say(`reading ${thread.length} reply(s) on this report`);
    ...
    const brief = [
      ...
      thread.length
        ? [
            '',
            'The conversation on this report so far. A previous run may have asked a question;',
            'these are the answers. Treat them as the most reliable detail you have:',
            ...thread.map(
              (n) => `  ${n.author === 'agent' ? 'You previously' : 'The maintainer'}: ${n.body}`,
            ),
          ].join('\n')
        : '',
      '',
    ...
    const paths = [...touched];
    if (!paths.length) {
      const text = summary || 'The agent did not find a change it was confident in.';
      await patchRun(runId, {
        status: 'blocked',
        summary: text,
        finished_at: new Date().toISOString(),
      });
      // Into the thread as well as the run, so the reasoning sits in the
      // conversation a reply answers rather than in a run record nobody
      // reading the board would think to open.
      await tellmeAddNote(report.id, {
        author: 'agent',
        body: text,
