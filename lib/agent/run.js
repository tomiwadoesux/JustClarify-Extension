...
import { CATEGORIES, categoryOf, autonomyOf, pathAllowed, verificationPlan } from './policy';
...
export async function runAgent({ runId, report }) {
  ...
  try {
    ...
    const category = await classify(report);
    ...
    if (prUrl) {
      await tellmeDb(`jc_reports?id=eq.${report.id}`, {
        method: 'PATCH',
        body: { fix_state: 'proposed', fix_pr_url: prUrl, fix_branch: branch },
      });
      await log.say('pull request opened, waiting on a human');
    }
